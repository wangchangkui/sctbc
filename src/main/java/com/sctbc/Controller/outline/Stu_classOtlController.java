...
@Api(tags = "班级接口")
@RestController
@RequestMapping("/outline/stuclass")
public class Stu_classOtlController {
    @Autowired
    private Stu_classOtlServer stu_classOtlServer;
    @Autowired
    private TokenServer tokenServer;


    @ApiOperation(value = "根据教师ID查询学生班级信息")
    @ApiImplicitParams({
    ...
    @GetMapping("/queryStuClassByid")
    public ReturnValue queryStuOtlClass(@RequestParam("id") String id, @RequestParam("token") String token) {
        if (tokenServer.selectToken(token)==null) {
            return new ReturnValue(400, "你还未登录");
        }

        return new ReturnValue(200, "查询成功", stu_classOtlServer.queryStuOtlClass(id));
    }
}
